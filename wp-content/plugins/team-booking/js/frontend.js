jQuery(document).ready(function ($) {
    ...
    /*
     * Changing the timezone of the frontend calendar.
     */
    function tbkChangeTimezone(calendar, $calendar, $clicked) {
        if (calendar.hasClass('loading')) return false;
        // We're put the calendar area in a loading state
        calendar.toggleClass('tbk-loading');
        // Let's grab the frontend calendar parameters
        ...
        $.post(
            TB_Ajax.ajax_url,
            {
                action  : 'tbajax_action_change_timezone',
                timezone: $clicked.attr('data-timezone'),
                params  : params
            },
            function (response) {
                response = tbUnwrapAjaxResponse(response);
    ...
    function tbkLoadReservationForm(calendar, $calendar, $clicked) {
        var clicked = $clicked;
        if (calendar.find('.tbk-schedule-slot.tb-book').hasClass('tbk-loading')) {
            return false;
        }
            ...
            function (response) {
                ...
                if (address.length !== 0) {
                    ...
                    if (typeof google !== 'undefined') {
                        ...
                        $reservation_slider.find(".tbk-segment.tbk-map").html('Google Maps library not loaded.');
                    }
                }
                $slider.goToSlide(2);
    ...
    tbUpdateTotalPrice = function ($item_selected) {
        ...
        total_disc = total_disc.toFixed(2);
    ...
    function tbkCalendar(calendar) {
        ...
        this.init = function () {
            ...
            calendar
                ...
                .on("click keydown", ".tbk-schedule-slot.tb-book-advice", function (e) {
                    e.stopPropagation();
                    if (e.which == 13 || e.which == 32 || e.which == 1) {
                ...
                .on('change', '.tbk-ticket-value', function () {
                    ...
                    $(this).closest('.tbk-reservation-form-container').find('form input[name=quantity]').val(selected_number);
                })
    ...
    jQuery.tbkSliderGet = function ($container) {
        return new tbkSlider($container);
    }

    /*
     * Main calendar instances
     */
    $('.calendar_main_container, .calendar_widget_container').each(function () {
        var calendar = new tbkCalendar($(this).find('.tb-frontend-calendar'));
        calendar.init();
     ...
     */
    $('.calendar_main_container, .calendar_widget_container').on("click keydown", ".tbk-main-calendar-settings .tbk-setting-button .tbk-menu-item", function (e) {
        if (e.which != 13 && e.which != 1) {
            return;
        }
    ...
    $(document).click(function () {
        var $menu = $('.calendar_main_container, .calendar_widget_container').find('.tbk-menu');
